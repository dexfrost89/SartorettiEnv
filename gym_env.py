...
class SartorettiEnvGym(gym.Env):
    def __init__(self, env_size=None, obstacle_density=None, num_agents=1):
        super(SartorettiEnvGym, self).__init__()
        if env_size is None:
        ...
        self.action_space = gym.spaces.Tuple([gym.spaces.Discrete(5)] * self.num_agents)
        self.observation_space = gym.spaces.Tuple([gym.spaces.Dict(
            {'obstacles': gym.spaces.Box(low=0.0, high=1.0, shape=(self.env_size, self.env_size), dtype=np.float32),
             'agents': gym.spaces.Box(low=0.0, high=1.0, shape=(self.env_size, self.env_size), dtype=np.float32),
             'targets': gym.spaces.Box(low=0.0, high=1.0, shape=(self.env_size, self.env_size), dtype=np.float32),
             'goal': gym.spaces.Box(low=0.0, high=1.0, shape=(self.env_size, self.env_size), dtype=np.float32),
             'goal_vector': gym.spaces.Box(low=-1.0, high=1.0, shape=(2,), dtype=np.float32),
             'agents': gym.spaces.Box(low=0.0, high=np.Inf, shape=(1,), dtype=np.float32)})])
        self.generate_env()
